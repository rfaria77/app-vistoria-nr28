...
# ReportLab para emissão do Laudo PDF Pericial
...
# ---------------------------------------------------------
# Carimbo Forense e Tratamento de Imagem
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# ReportLab: Canvas com Numeração e Logo VistorIA no Rodapé
# ---------------------------------------------------------
...
def gerar_pdf_pericial_completo(dados, evidencias, logo_cons=None, ass_acomp_pil=None):
    buf = io.BytesIO()
    ...
    # CABEÇALHO: Logo da Consultoria à esquerda
    texto_cab = [
    ...
    # Apontamentos com fotos anexadas
    for idx, ev in enumerate(evidencias, 1):
        ...
        elementos.append(Paragraph(f"<b>#{idx} - [{tag}] - {ev['nr']} (Item {ev['item_nr']})</b>", ParagraphStyle('TitEv', parent=styles['Heading3'], textColor=colors.HexColor(cor), fontSize=9.5)))
    ...
    linha_ass = [Paragraph("<br/><br/>", cell_td_center), img_ass_acomp]
    linha_nomes = [
...
def index():
    ...
    ui.add_body_html("""
        <script>
            ...
            function abrirCameraNativa() {
                var inputCam = document.getElementById('input-camera-direta');
                if (inputCam) {
                    inputCam.click();
                }
            }
    ...
    if 'sessao' not in app.storage.user:
        app.storage.user['sessao'] = {
            'autenticado': False,
            'usuario': '',
            'perfil': '',
            'visao': 'vistoria',
    ...
    sessao = app.storage.user['sessao']
            ...
            with ui.card().classes('w-full max-w-sm mx-auto p-6 mt-12 text-center shadow-lg'):
                ...
                def tentar_login():
                    ...
                    if row:
                        ...
                        sessao['usuario'] = row[0]
                        sessao['perfil'] = row[1]
                        sessao['inspetor'] = f"{row[0].capitalize()} (SST)"
    ...
    def renderizar_sistema():
        ...
        with container_principal:
            ...
            with ui.row().classes('w-full items-center justify-between bg-slate-900 text-white px-4 py-3 rounded-xl mb-4'):
                ...
                with ui.row().classes('items-center gap-2'):
                    if sessao['perfil'] == 'Admin':
                        btn_label = "⚙️ ADM" if sessao['visao'] == 'vistoria' else "📋 Vistoria"
                        def toggle_visao():
                            sessao['visao'] = 'admin' if sessao['visao'] == 'vistoria' else 'vistoria'
                            renderizar_sistema()
            ...
            # VISÃO ADM
            # =====================================================
            if sessao['visao'] == 'admin' and sessao['perfil'] == 'Admin':
                ...
                with ui.tab_panels(t_adm, value=tab_emp).classes('w-full p-2'):
                    ...
                    with ui.tab_panel(tab_hist):
                        ui.label("Histórico de Laudos").classes('font-bold mb-2')
                        relatorios = listar_relatorios_db()
                        if relatorios:
                            for r in relatorios:
                                with ui.card().classes('w-full mb-2 p-3'):
                                    ui.label(f"#{r[0]} - {r[2]} ({r[1]})").classes('font-bold')
                                    def baixar(id_r=r[0], emp=r[2]):
                                        d = obter_pdf_relatorio_db(id_r)
                                        if d:
                                            ui.download(d[0], f"Laudo_{id_r}_{emp.replace(' ', '_')}.pdf")
                                    ui.button("⬇️ Baixar PDF", on_click=baixar).props('dense outline')
                        else:
            ...
            # =====================================================
                ...
                with ui.tab_panels(tabs_v, value=sessao['aba_ativa']).classes('w-full p-2'):
                    # ETAPA 1: Identificação & Cadastro Rápido
                    with ui.tab_panel(v1):
                        ...
                        sel_emp = ui.select(nomes_emp, value=sessao['empresa'] if sessao['empresa'] in nomes_emp else nomes_emp[0], label="Empresa a ser auditada:").classes('w-full')
                        ...
                        with ui.dialog() as dialog_novo_cliente, ui.card().classes('w-full max-w-sm p-4'):
                            ...
                            def salvar_cliente_rapido():
                                if cad_nome.value.strip():
                                    ...
                                    sessao['empresa'] = cad_nome.value.strip()
                        ...
                        txt_insp = ui.input("Auditor / Responsável Técnico:", value=sessao['inspetor']).classes('w-full')
                        txt_reg = ui.input("Registro Profissional (MTE / CREA):", value=sessao['reg_inspetor']).classes('w-full')
                        sel_faixa = ui.select(list(TABELA_MULTAS_SEGURANCA.keys()), value=sessao['faixa'], label="Quadro de Funcionários (NR 28):").classes('w-full')
                        txt_wpp = ui.input("WhatsApp do Gestor da Empresa:", value=sessao['wpp']).classes('w-full')

                        def avancar_etapa2():
                            sessao['empresa'] = sel_emp.value
                            sessao['inspetor'] = txt_insp.value
                    ...
                    # ETAPA 2: Apontamentos com IA e Câmera Direta
                    with ui.tab_panel(v2):
                        tot_m = sum(e['valor_max'] for e in sessao['evidencias'] if e['status'] == 'Não Conformidade')
                        tot_e = sum(e['valor_max'] for e in sessao['evidencias'] if e['status'] == 'Conformidade')
                        ...
                        def handle_foto(e):
                            img_raw = Image.open(io.BytesIO(e.content.read()))
                            img_carimbada = otimizar_e_carimbar(img_raw)
                            sessao['foto_temporaria'] = img_carimbada
                            ui.notify("✅ Foto capturada com Carimbo Forense!", type='positive')
                            renderizar_sistema()

                        ui.html('<input type="file" id="input-camera-direta" accept="image/*" capture="environment" style="display:none;" />')
                        ui.button("📸 Tirar Foto com Câmera", on_click=lambda: ui.run_javascript("abrirCameraNativa()")).classes('w-full bg-slate-800 text-white font-bold h-11 my-1')
                        ui.upload(on_upload=handle_foto, max_files=1, auto_upload=True).classes('hidden')

                        if sessao['foto_temporaria']:
                            b_f = io.BytesIO()
                            sessao['foto_temporaria'].save(b_f, format='JPEG')
                            ui.image(f"data:image/jpeg;base64,{base64.b64encode(b_f.getvalue()).decode()}").classes('w-full rounded border my-2')
                        ...
                        def salvar_apontamento():
                            ...
                            val_min, val_max = calcular_multa(linha['infracao'], sessao['faixa'], linha['tipo'])

                            sessao['evidencias'].append({
                                'nr': sel_nr_ap.value,
                                'item_nr': it_cod,
                        ...
                        # Listagem
                        if sessao['evidencias']:
                            ui.label(f"Apontamentos Registrados ({len(sessao['evidencias'])}):").classes('font-bold text-sm mt-4')
                            for i, ev in enumerate(sessao['evidencias']):
                    ...
                    with ui.tab_panel(v3):
                        ...
                        txt_ac_cargo = ui.input("Cargo / Função:", value=sessao['acomp_cargo']).classes('w-full')
                        ...
                        tot_multa_fim = sum(e['valor_max'] for e in sessao['evidencias'] if e['status'] == 'Não Conformidade')
                        tot_econ_fim = sum(e['valor_max'] for e in sessao['evidencias'] if e['status'] == 'Conformidade')
                        qtd_nc_fim = sum(1 for e in sessao['evidencias'] if e['status'] == 'Não Conformidade')
                        link_wpp = gerar_link_whatsapp(sessao.get('wpp', '34999990000'), sessao['empresa'], tot_multa_fim, tot_econ_fim, qtd_nc_fim)
                        ...
                        async def emitir_pdf_final():
                            sessao['acomp_nome'] = txt_ac_nome.value
                            sessao['acomp_cargo'] = txt_ac_cargo.value

                            b64_sig = await ui.run_javascript("exportarAssinaturaPNG()")
                            ass_pil = None
                            if b64_sig and "base64," in b64_sig:
                                img_bytes = base64.b64decode(b64_sig.split("base64,")[1])
                                ass_pil = tratar_imagem_assinatura(Image.open(io.BytesIO(img_bytes)))
                            ...
                            pdf_bytes = gerar_pdf_pericial_completo({
                                ...
                                'inspetor': sessao['inspetor'],
                                'reg_inspetor': sessao['reg_inspetor'],
                                'acomp_nome': sessao['acomp_nome'],
                                'acomp_cargo': sessao['acomp_cargo'],
                                'faixa': sessao['faixa'],
                                'data': data_hoje
                            }, sessao['evidencias'], logo_cons=logo_cons, ass_acomp_pil=ass_pil)

                            salvar_relatorio_db(data_hoje, sessao['empresa'], sessao['inspetor'], len(sessao['evidencias']), 0, tot_multa_fim, 0, tot_econ_fim, pdf_bytes)
                            ui.download(pdf_bytes, f"Laudo_SST_{sessao['empresa'].replace(' ', '_')}.pdf")
